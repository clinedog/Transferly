...
| 18 | Partially implemented | Explicit loading/error/unknown/reconciliation states, in-app notifications, persisted per-channel delivery state, auditable attempt metadata, retryable delivery records, authenticated persisted notification category/channel preferences, contextual support handoff, and persisted support tickets with transaction context/history exist. Telegram/email/webhook delivery workers, provider-backed channel adapters, agent assignment, and external ticket escalation still need implementation. |
| 19 | Partially implemented | Admin transaction center, receipt vault, search/filtering, CSV export, transaction-linked support context, and authenticated bounded transaction-activity list/detail read models are present. The Mini App now consumes authoritative funding, top-up, receipt, invoice, and payout records with provider references, sanitized audit/provider timelines, related ledger records, and webhook metadata surfaced in detail. User-scoped activity search now supports provider, currency, and validated date-window filters; cross-entity joins and broader financial-search coverage remain to be added. |
