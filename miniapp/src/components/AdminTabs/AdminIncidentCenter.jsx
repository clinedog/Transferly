...
export default function AdminIncidentCenter() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 lg:grid-cols-2">{state.incidents.map((incident) => <SurfaceCard as="article" key={incident.id} className="p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-[var(--tg-hint-color)]">{incident.display_name}</p><h3 className="mt-2 text-xl font-black text-[var(--tg-text-color)]">{incident.affected_operation}</h3></div><span className={`rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] ${STATUS_STYLES[incident.status] || STATUS_STYLES.INVESTIGATING}`}><AlertTriangle className="mr-1 inline" size={13} />{incident.status}</span></div><p className="mt-4 text-sm text-[var(--tg-subtitle-text-color)]">{incident.impact}</p><div className="mt-4 grid grid-cols-2 gap-3 text-xs"><div className="rounded-2xl bg-[var(--tg-secondary-bg-color)] p-3"><p className="font-bold text-[var(--tg-hint-color)]">Health score</p><p className="mt-1 text-lg font-black text-[var(--tg-text-color)]">{incident.evidence.health_score ?? 'Unavailable'}</p></div><div className="rounded-2xl bg-[var(--tg-secondary-bg-color)] p-3"><p className="font-bold text-[var(--tg-hint-color)]">Open issues</p><p className="mt-1 text-lg font-black text-[var(--tg-text-color)]">{incident.evidence.unresolved_issues}</p></div></div>{incident.next_actions?.length ? <div className="mt-4 rounded-2xl border border-amber-300/20 bg-amber-300/10 p-3 text-xs font-bold text-amber-100">{incident.next_actions.slice(0, 3).join(' ')}</div> : null}</SurfaceCard>)}</div>
    </div>
  );
}
