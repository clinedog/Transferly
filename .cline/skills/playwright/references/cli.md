...
From the Transferly repository root, use the bundled wrapper unless the CLI is already installed globally:

```bash
export PWCLI="$PWD/.cline/skills/playwright/scripts/playwright_cli.sh"
"$PWCLI" --help
```

The repository-local `.cline/skills` path is shared by Cline and Codex through the `.codex/skills` compatibility bridge.
